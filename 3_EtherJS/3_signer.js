    ...
    const object = require("ethers");
    // b. Create a Goerli provider.
    const providerKey = process.env.INFURA_KEY; 
    const goerlyInfuraAddress = `${process.env.INFURA_GOERLI}${providerKey}`;
    const goerliProviders = new object.JsonRcpProvider(goerlyInfuraAddress);
    ...
    console.log('This is a public key to metamask address 1', signer.address);
    ...
    if(signer.address == process.env.METAMASK_1_ADDRESS){
        console.log('same');
    }else{
    ...
    const sign = async (message = 'Hello world') => {
        ...
        // verify message returns the public key of the signer
        const verifiedSigner = ethers.verifyMessage(message, signature);

        if(verifiedSigner == signer.address){
            console.log('correct');
            console.log('signer is', verifiedSigner);
        }else{
            console.log("false shit you shit")
        }
    };
    ...
    const connectToBlockchain = async() => {
        let connection = await signer.connect(goerliProvider);
        console.log(connection);

        // let nonce = await connection.getNonce();
    ...
    const sendTransaction = async () => {
        let balance1 = await goerliProvider.getBalance(account2);
