...
console.log('Hardhat\'s default networkyyy:', hre.config.defaultNetwork);
...
// Your code here!
console.log("wrapper version", hre.ethers.version);
// return;
...
async function main() {
  ...
  const myHhSigner = hardhatSigners[0];
  console.log("First signer ", hardhatSigners[0].address);
  ...
  const getMyContract = async() => {
    const newContract = await hre.ethers.getContractAt(contractName, contractAddress, hardhatSigners[0]);
    console.log('New contract ', newContract.address);
    // Your code here!
    return newContract;
  }

  const contractLock2 = await getMyContract();
  ...
  const readContract = async (lockContract = contractLock2) => {
      ...
      console.log('Owner of Lock 2 contract is: ', owner);
    // Print the owner of the lock.
   
      let unlockTime = await lockContract.unlockTime();
  ...
  const withdrawAttempt1 = async (lockContract = contractLock2) => {
    let balance1 = await myHhSigner.getBalance();
    balance1 = ethers.utils.formatEther(balance1);
    console.log("balance 1 is: ", balance1);  
    
    console.log("withdrawing from lock2");
    await lockContract.withdraw();

    let balance2 = await myHhSigner.getBalance();
    balance2 = ethers.utils.formatEther(balance2);
    console.log("balance 1 is: ", balance2);  
  };
